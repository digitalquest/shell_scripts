...
function exec_command($command)
{
    ...
    foreach ($output as $line) {
        log_append('SHELL testpush: ' . $line);
        if ($debug) 
	  echo $line. '<br/>';
    }
}
...
log_append('Launching shell test script...');
exec_command('sh ' . $hookfile);
log_append('Shell test script finished');

if ($debug)
  echo '<br/><hr/>The end --- '.date('Y-m-d H:i:s');
